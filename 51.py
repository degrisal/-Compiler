...
class Lexer:
    def __init__(self, keywords, delimiters):
        ...
        self.token_specification = [
            ('NUMBER',   r'\d+(\.\d+)?([eE][+-]?\d+)?|\.\d+([eE][+-]?\d+)?'),  # Изменено регулярное выражение для чисел
            ('BINARY',   r'\d+B'),
            ('OCTAL',    r'\d+O'),
            ('HEX',      r'\d+H'),
...
class Interpreter:
    ...
    def handle_input(self, line):
        ...
        for var_name in parts:
            ...
            if isinstance(self.variables[var_name], int):
                if value.endswith("B"):  # Двоичное число
                    try:
                ...
                elif value.endswith("O"):  # Восьмеричное число
                    try:
                ...
                elif value.endswith("H"):  # Шестнадцатеричное число
                    try:
    ...
    def handle_for(self, line, lines, index):
        ...
        if end_expression.endswith("#"):
            end = int(end_expression[:-1])
        elif end_expression.endswith("@"):
...
def clear():
    global lexer, interpreter, file_path
    lexer = None
    interpreter = None
    file_path = None
